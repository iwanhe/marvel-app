import { ALL_DAYS, WEEKDAYS, WEEKEND, EMPTY_WEEK } from "../../constants";
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 18px", borderBottom: "1px solid #F5F5F5" }}>
          <div>
            ...
            <div style={{ fontFamily: "'Nunito', sans-serif", fontSize: 11, color: "#AAA", marginTop: 2 }}>
              {showWeekend ? "Semua hari (Senin–Minggu)" : "Hanya hari kerja (Senin–Jumat)"}
            </div>
          </div>
        ...
        <div style={{ padding: "14px 18px", background: "#FAFAFA" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {ALL_DAYS.map((d) => {
              ...
              return (
                <div key={d} style={{ padding: "4px 10px", borderRadius: 8, background: active ? (iw ? "#37474F" : "#1a1a2e") : "#EEE", color: active ? "#fff" : "#BBB", fontFamily: "'Nunito', sans-serif", fontWeight: 700, fontSize: 11, transition: "all 0.3s" }}>
                  {d}
                </div>
              );
            })}
      ...
      <div style={{ borderRadius: 14, background: "#fff", border: "1px solid #EBEBEB", padding: "14px 18px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 7, marginBottom: 14 }}>
          {Object.keys(weekSchedules).map((wi) => {
            ...
            return (
              <div key={wi} onClick={() => setWeekIndex(wiNum)} style={{ borderRadius: 11, border: isCurrent ? "2px solid #1a1a2e" : "1px solid #EBEBEB", background: isCurrent ? "#1a1a2e" : "#FAFAFA", padding: "10px 13px", cursor: "pointer", transition: "all 0.2s" }}>
                ...
                <div style={{ display: "flex", gap: 4, marginTop: 6, flexWrap: "wrap" }}>
                  {WEEKDAYS.map((d) => (
                    <span key={d} style={{ fontFamily: "'Nunito', sans-serif", fontSize: 9, fontWeight: 700, background: isCurrent ? "rgba(255,255,255,0.15)" : "#EBEBEB", color: isCurrent ? "#fff" : "#777", borderRadius: 4, padding: "1px 5px" }}>
                      {wd[d]}
                    </span>
                  ))}
                </div>
        ...
        <button onClick={() => addWeek(nextIdx)} style={{ width: "100%", padding: "12px", borderRadius: 12, border: "2px dashed #1a1a2e40", background: "#FAFAFA", color: "#1a1a2e", fontFamily: "'Nunito', sans-serif", fontWeight: 800, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          ➕ Tambah Minggu Baru
          <span style={{ fontSize: 11, fontWeight: 600, color: "#888" }}>({nextRange.start} — {nextRange.end})</span>
        </button>
